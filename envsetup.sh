...
function build_build_var_cache()
{
    ...
    cached_vars=(`cat $T/build/envsetup.sh $T/vendor/x01/build/envsetup.sh | tr '()' '  ' | awk '{for(i=1;i<=NF;i++) if($i~/_get_build_var_cached/) print $(i+1)}' | sort -u | tr '\n' ' '`)
    cached_abs_vars=(`cat $T/build/envsetup.sh $T/vendor/x01/build/envsetup.sh | tr '()' '  ' | awk '{for(i=1;i<=NF;i++) if($i~/_get_abs_build_var_cached/) print $(i+1)}' | sort -u | tr '\n' ' '`)
...
{
    if (echo -n $1 | grep -q -e "^x01_") ; then
        X01_BUILD=$(echo -n $1 | sed -e 's/^x01_//g')
    else
        X01_BUILD=
    fi
    export X01_BUILD
...
. $ANDROID_BUILD_TOP/vendor/x01/build/envsetup.sh
